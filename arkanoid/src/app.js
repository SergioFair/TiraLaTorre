...
var GameLayer = cc.Layer.extend({
    ...
    ctor: function () {
        ...
        pelotaShape = new cp.CircleShape(body, this.spritePelota.width / 2, cp.vzero);
        this.space.addShape(pelotaShape);
        ...
        if (instancia.tiempo == 0) {
            ...
            body.applyImpulse(
                cp.v(event.getLocationX() - body.p.x, event.getLocationY() - body.p.y), cp.v(0, 0));
        }
        ...
        if (this.arrayBloques.length > 0) {
            ...
            if (this.tiempo != 0 && (new Date().getTime() - this.tiempo) > 5000 && todosSinMoverse) {
                this.intentos++;
                this.restaurarPosicionPelota();
                this.tiempo = 0;
    ...
    }, restaurarPosicionPelota: function () {

        var body = this.spritePelota.getBody();
        this.space.removeShape(pelotaShape);
